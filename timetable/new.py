...
#print(even_sem)
teachers=pd.read_csv('teachers1.csv').to_dict() # import csv file
#print(teachers)
...
while(True):
    n=input("Enter the class name or Faculty name(classes - 2s,4s,6s,2d,4d,6d,2t,4t,6t): ")
    if n=="2s":
        root = Tk()
        t = Timetable(root,list2ss)
        root.mainloop()
    elif n=="4s":
        root = Tk()
        t = Timetable(root,list4ss)
        root.mainloop()
    elif n=="6s":
        root = Tk()
        t = Timetable(root,list6ss)
        root.mainloop()
    elif n=="2d":
        root = Tk()
        t = Timetable(root,list2ds)
        root.mainloop()
    elif n=="4d":
        root = Tk()
        t = Timetable(root,list4ds)
        root.mainloop()
    elif n=="6d":
        root = Tk()
        t = Timetable(root,list6ds)
        root.mainloop()
    elif n=="2t":
        root = Tk()
        t = Timetable(root,list2t)
        root.mainloop()
    elif n=="4t":
        root = Tk()
        t = Timetable(root,list4t)
        root.mainloop()
    elif n=="6t":
        root = Tk()
        t = Timetable(root,list6t)
        root.mainloop()
    elif n in teachertt.keys():
        ll=[]
        for i in range(0,5):
            l=[days[i]]
            for j in range(0,5):
                if str(i)+','+str(j) in teachertt[n].keys():
                    l.append(teachertt[n][str(i)+','+str(j)])
                else:
                    l.append('-')
            ll.append(l)
